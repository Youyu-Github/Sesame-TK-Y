    ...
    private fun recordFarmGame(gameType: GameType) {
        ...
        try {
            do {
                try {
                    // 1. 初始化游戏
                    val initResponse = AntFarmRpcCall.initFarmGame(gameType.name)
                    val initJo = JSONObject(initResponse)
                    
                    if (!ResChecker.checkRes(TAG + "初始化庄园游戏失败:", initJo)) {
                        Log.runtime(TAG, "初始化庄园游戏失败，响应: $initJo")
                        retryCount++
                    ...
                    // 3. 记录游戏
                    val recordResponse = AntFarmRpcCall.recordFarmGame(gameType.name)
                    val recordJo = JSONObject(recordResponse)
                    ...
                    // 4. 处理奖励信息
                    val award = StringBuilder()
                    if (recordJo.has("awardInfos")) {
                        val awardInfos = recordJo.getJSONArray("awardInfos")
                        for (i in 0 until awardInfos.length()) {
                            val awardInfo = awardInfos.getJSONObject(i)
                            award.append(awardInfo.getString("awardName"))
                                .append("*")
                    ...
                    if (recordJo.has("receiveFoodCount")) {
                        award.append(";肥料*").append(recordJo.getString("receiveFoodCount"))
                    }
                    ...
                    Log.runtime(TAG, "JSON解析异常: ${e.message}")
                    retryCount++
                    if (retryCount >= maxRetries) break
                } catch (e: Exception) {
                    Log.runtime(TAG, "游戏处理异常: ${e.message}")
                    retryCount++
                    if (retryCount >= maxRetries) break
                } finally {
                    GlobalThreadPools.sleep(2000)
            ...
            Log.runtime(TAG, "recordFarmGame严重错误:")
            Log.printStackTrace(TAG, t)
        }
    }
